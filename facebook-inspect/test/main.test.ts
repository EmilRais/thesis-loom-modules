...
describe("operation", () => {
    ...
    it("should fail if token has not been specified", () => {
        const abstractOperation: Operation = { module: "facebook-inspect", token: null };
        return prepareOperation(abstractOperation)
            .then(() => Promise.reject("Expected failure"))
            .catch(error => error.should.equal("facebook-inspect expected a token"));
    });

    it("should fail if no user token", () => {
        const abstractOperation: Operation = { module: "facebook-inspect", token: "response.locals.void" };
        return prepareOperation(abstractOperation)
            .then(operation => {
    ...
    it("should store inspected user token", () => {
        const abstractOperation: Operation = { module: "facebook-inspect", token: "request.body.token" };
        return prepareOperation(abstractOperation)
            .then(operation => {
                return new Promise((resolve, reject) => {
                    express()
