...
describe("Facebook", () => {
    ...
    describe("extendToken", () => {
        it("should fail if invalid credentials", () => {
            const facebook = new Facebook("invalid-app-id", "invalid-secret");
            return facebook.extendToken(userToken)
                .catch(error => error.response)
                .then(response => {
                    response.text.should.include("Invalid Client ID");
                });
        });

        it("should fail if no token", () => {
            const facebook = new Facebook("1092068880930122", "470f440e050eb59788e7178c86ca982f");
            return facebook.extendToken(null)
                .catch(error => error.response)
                .then(response => {
        ...
        it("should fail if invalid token", () => {
            const facebook = new Facebook("1092068880930122", "470f440e050eb59788e7178c86ca982f");
            return facebook.extendToken("invalid-token")
                .catch(error => error.response)
                .then(response => {
                    response.text.should.include("Invalid OAuth access token.");
                });
        });

        it("should return extended token if valid token", () => {
            const facebook = new Facebook("1092068880930122", "470f440e050eb59788e7178c86ca982f");
            return facebook.extendToken(userToken)
                .then(extendedToken => {
                    extendedToken.access_token.should.be.string;
                    extendedToken.token_type.should.equal("bearer");

                    const fiftyDays = 50 * 60 * 60 * 24;
                    extendedToken.expires_in.should.be.greaterThan(fiftyDays);
                });
        });
    });
